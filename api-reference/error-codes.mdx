---
title: "Jupico API Error Codes: Causes and Remediation Steps"
sidebarTitle: "Error Codes"
description: "Catalog of stable error codes returned by the Jupico API, including cause, remediation steps, and mappings for payments, onboarding, and payouts."
---
...
| `400` | `provisioning_no_service_selected` | No service selected. Enable at least one service in `services` — `card`, `ach`, or both. |
| `400` | `provisioning_service_not_available` | A selected service is not available for the service provider product. Enable only services the product identified by `spProductId` offers. |
...
| Receipt | `400` | `receipt_format_unsupported` | Receipt response format not supported. |
