...
eCheck transactions expose the same receipt endpoint as card transactions. The payload returns `paymentMethod.accountType` and the bank account's last 4 digits in `paymentMethod.last4`; the card-only `brand` and `cardHolderName` are omitted. See [Receipts](/docs/payments/card-transactions#receipts) for the amount rules and error behavior.
