...
<Warning>
  ...
  Do not retry a failed transaction with the same OTT. To retry — for example after a soft decline — have the customer resubmit the web component to generate a fresh OTT, then call the transaction endpoint with the new token.
</Warning>
